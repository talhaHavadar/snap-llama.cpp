...
choose_backend() {
  ...
  case "$req" in
    ...
    auto)
      ...
      echo ""; return ;;
    *)
      echo "llama-cpp: invalid backend='$req' (use cpu|hip|cuda|vulkan|auto)" >&2
      exit 2 ;;
  esac
}
...
if [ $# -eq 0 ]; then
  ...
  echo "  Configure persistently: sudo snap set llama-cpp backend={cpu,hip,cuda,vulkan,auto}"
